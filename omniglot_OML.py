...
class OML(nn.Module):
    ...
    def _inner_loop(self, images, labels, train):
        ...
        temp_class_scores = self._forward(images, parameters)
        temp_loss = F.cross_entropy(input= temp_class_scores, target= labels)
        each_gradients = torch.autograd.grad(temp_loss, inputs=list(parameters.values()), create_graph=train)


        # Update W parameters
        with torch.no_grad():
            for parameter_name, gradient in zip(parameters.keys(), each_gradients):
                if "conv" in parameter_name:
                    parameters[parameter_name] -= self._inner_lrs[parameter_name] * gradient
            
        
        with torch.no_grad():
            class_scores = self._forward(images, parameters)
            accuracies = self.cal_accuracy(class_scores, labels)
    ...
    def _outer_step(self, task_batch, train):
        ...
        for task in task_batch:
            ...
            batch_size = 5 
            num_images = images_support.size(0) 
            num_batches = (num_images + batch_size - 1) // batch_size
            
            with torch.no_grad():
                start_parameters = {k: torch.clone(v) for k, v in self._meta_parameters.items()}
                start_support_class_scores = self._forward(images_support, start_parameters)
                accuracies_support_batch.append(self.cal_accuracy(start_support_class_scores, labels_support))
            ...
            for batch_idx in range(num_batches):
                ...
                ## TAKE OUT THE BATCH SUPPORT 
                batch_images_support = images_support[0:end_idx]
                batch_labels_support = labels_support[0:end_idx]
                
                ## SAME AS THE QUERY
                batch_images_query = images_query[0:end_idx]
                batch_labels_query = labels_query[0:end_idx] 

                ## RUN INNER STEPS FOR EACH OF THE BATCH IN IT
                
                parameters, accuracies_support_task = self._inner_loop(batch_images_support, batch_labels_support, train)
                support_accuracy.append(accuracies_support_task)
                
                ## USE BACK THE PARAMETERS FOR UPDATING THE QUERY BATCH
                batch_class_scores = self._forward(batch_images_query, parameters)
                batch_labels_loss = F.cross_entropy(input= batch_class_scores, target=batch_labels_query)
                
                query_batch_gradients = torch.autograd.grad(batch_labels_loss, inputs=list(parameters.values()), create_graph=train)

                with torch.no_grad():
                    for parameter_name, gradient in zip(parameters.keys(), query_batch_gradients):
                        if "conv" not in parameter_name:
                            parameters[parameter_name] -= self._outer_lr  * gradient
            
            
            accuracies_support_batch.append(np.mean(support_accuracy))
            with torch.no_grad():
                final_support_class_scores = self._forward(images_support, parameters)
                accuracies_support_batch.append(self.cal_accuracy(final_support_class_scores, labels_support))
            
            
            final_query_class_scores = self._forward(images_query, parameters)
            outer_loss_batch.append(F.cross_entropy(input=final_query_class_scores, target=labels_query))

            
            accuracy_query_batch.append(self.cal_accuracy(final_query_class_scores, labels_query))
        ...
        return outer_loss, accuracies_support_batch , accuracy_query
    ...
    def test(self, dataloader_test):
        """Evaluate the OML on test tasks.
...
def main(args):
    ...
    if log_dir is None:
        log_dir = f'./logs/OML/omniglot.way_{args.num_way}.support_{args.num_support}.query_{args.num_query}.inner_steps_{args.num_inner_steps}.inner_lr_{args.inner_lr}.learn_inner_lrs_{args.learn_inner_lrs}.outer_lr_{args.outer_lr}.batch_size_{args.batch_size}'
    ...
    if not args.test:
        ...
        oml.sample_test(dataloader_test)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Train a OML!')
